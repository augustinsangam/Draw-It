...
export abstract class SelectionLogicBase extends ToolLogicDirective
  ...
  private resizeVisualisationRectangle(point: Point, mouseOffset: Util.Offset): void {
    ...
    if (!!x && !!y && !!width && !!height) {
      refPoint1 = new Point(+x, +y);
      refPoint2 = new Point(+x + +width, +y + +height);
    ...
    const xP1 = Math.min(refPoint1.x + ((this.mouse.left.selectedElement < 2) ? mouseOffset.x : 0), refPoint2.x);
    const xP2 = Math.max(refPoint2.x + ((this.mouse.left.selectedElement > 1) ? mouseOffset.x : 0), refPoint1.x);
    const p1 = new Point(xP1, refPoint1.y + ((this.mouse.left.selectedElement < 2) ? mouseOffset.y : 0));
    const p2 = new Point(xP2, refPoint2.y + ((this.mouse.left.selectedElement > 1) ? mouseOffset.y : 0));
