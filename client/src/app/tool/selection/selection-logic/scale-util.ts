...
export class ScaleUtil {
    ...
    : void {
        ...
        if (refPoint2.x - refPoint1.x <= 1 && splitedOffset[0].x !== 0) {
            if (splitedOffset[0].x < 0 && this.selectionLogic.mouse.left.selectedElement !== Util.CircleType.LEFT_CIRCLE) {
                this.selectionLogic.mouse.left.selectedElement = Util.CircleType.LEFT_CIRCLE;
                switched = true;
            } else if (splitedOffset[0].x > 0 && this.selectionLogic.mouse.left.selectedElement !== Util.CircleType.RIGHT_CIRCLE) {
                this.selectionLogic.mouse.left.selectedElement = Util.CircleType.RIGHT_CIRCLE;
                switched = true;
            }
            if (switched) {
        ...
        if (refPoint2.y - refPoint1.y <= 1 && splitedOffset[0].y !== 0) {
            if (splitedOffset[0].y < 0 && this.selectionLogic.mouse.left.selectedElement !== Util.CircleType.TOP_CIRCLE) {
                this.selectionLogic.mouse.left.selectedElement = Util.CircleType.TOP_CIRCLE;
                switched = true;
            } else if (splitedOffset[0].y > 0 && this.selectionLogic.mouse.left.selectedElement !== Util.CircleType.BOTTOM_CIRCLE) {
                this.selectionLogic.mouse.left.selectedElement = Util.CircleType.BOTTOM_CIRCLE;
                switched = true;
            }
            if (switched) {
    ...
    private drawVisualisationResising(mouseOffset: Offset, refPoint: Point[]): Point[] {
        const p1 = new Point(
            refPoint[0].x + ((this.selectionLogic.mouse.left.selectedElement === Util.CircleType.LEFT_CIRCLE) ? mouseOffset.x : 0),
            refPoint[0].y + ((this.selectionLogic.mouse.left.selectedElement === Util.CircleType.TOP_CIRCLE) ? mouseOffset.y : 0),
        );
        const p2 = new Point(
            refPoint[1].x + ((this.selectionLogic.mouse.left.selectedElement === Util.CircleType.RIGHT_CIRCLE) ? mouseOffset.x : 0),
            refPoint[1].y + ((this.selectionLogic.mouse.left.selectedElement === Util.CircleType.BOTTOM_CIRCLE) ? mouseOffset.y : 0),
        );
